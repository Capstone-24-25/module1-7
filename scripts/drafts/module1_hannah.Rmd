...
# Testing accuracy and rebuilding training/testing sets
new_model <- biomarker_clean %>% 
  select(group, any_of(lasso_proteins)) %>% 
  mutate(class = as.numeric(group == 'ASD')) %>% 
  select(-group)

set.seed(46400)
partitions <- new_model %>%
  initial_split(prop = 0.8)

bio_train <- training(partitions)
...
class_metric # accuracy of 0.8387
```
